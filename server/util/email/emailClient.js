...
let config = {
  ...
  sendEmail: process.env.sendEmail === 'true' || false,
...
module.exports.sendEmail = function(
  ...
  if (config.sendEmail) {
    ...
    if (config.debug || config.dev) {
      ...
      console.log('emailClient.js redirect');
      ...
      if (error) {
        console.error(
          new Date().toLocaleString() +
            ' [emailClient.js] Could not send email to : ' +
            to
        );
        return console.error(error);
      }
      console.log(
        new Date().toLocaleString() + ' Message %s sent: %s',
        info.messageId,
        info.response
      );
      // console.log(info);
    ...
    console.log(
      new Date().toLocaleString() +
        ' SEND_EMAIL=false , email is not sent to ' +
        to
    );
  }
};

...
let admins = json.parse(
  fs.readFileSync(path.join(__dirname, './env/administrators.json')).toString(),
  null,
  true
);
...
console.log(
  'loading env file from : ' + path.join(__dirname, './env/env.prod.json')
);
cfg = json.parse(
  fs.readFileSync(path.join(__dirname, './env/env.prod.json')).toString(),
  null,
  true
);
...
if (process.env.NODE_ENV === 'test' || process.env.NODE_ENV === 'debug') {
  ...
  console.log(
    'using env file from : ' + path.join(__dirname, './env/env.test.json')
  );

  // Spread in object literals is not support yet

  let cfg_test = json.parse(
    fs.readFileSync(path.join(__dirname, './env/env.test.json')).toString(),
    null,
    true
  );

  for (let key in cfg_test) {
    cfg[key] = cfg_test[key];
  }
...
if (process.env.NODE_ENV === 'dev' || process.env.NODE_ENV === 'development') {
  ...
  console.log(
    'using env file from : ' + path.join(__dirname, './env/env.dev.json')
  );
  let cfg_dev = json.parse(
    fs.readFileSync(path.join(__dirname, './env/env.dev.json')).toString(),
    null,
    true
  );

  for (let key in cfg_dev) {
    cfg[key] = cfg_dev[key];
  }
...
if (cfg.smsGateway === 'twilio' && cfg.sendSms) {
  ...
  if (!isTwilioConfigured) {
    let errorMessage =
      'twilioAccountSid, twilioAuthToken, and twilioSenderNumber must be set.';
...
if (cfg.emailGateway === 'mailjet' && cfg.sendEmail) {
  ...
  if (!isMailjetConfigured) {
    let errorMessage =
      'mailjet_email, mailjet_api, and mailjet_secret must be set.';
    throw new Error(errorMessage);
  }
}
...
if (cfg.database === 'oracle') {
  ...
  if (!isConfigured) {
    let errorMessage =
      'Oracle database require to set dbConfig.user dbConfig.password, and dbConfig.connectString in env json';
    throw new Error(errorMessage);
  }
}
...
if (cfg.database === 'postgres') {
  ...
  if (!isConfigured) {
    let errorMessage =
      'PostgreSQL require to set dbConfig.connectString in env json';
    throw new Error(errorMessage);
  }
}
...
if (cfg.database === 'firebase') {
  let mustSet = [
    cfg.firebaseConfig.type,
    cfg.firebaseConfig.project_id,
    cfg.firebaseConfig.private_key_id,
    cfg.firebaseConfig.private_key,
    cfg.firebaseConfig.client_email,
    cfg.firebaseConfig.client_id,
    cfg.firebaseConfig.auth_uri,
    cfg.firebaseConfig.token_uri,
    cfg.firebaseConfig.auth_provider_x509_cert_url,
    cfg.firebaseConfig.client_x509_cert_url,
  ];
  let isConfigured = mustSet.every(function(configValue) {
    return configValue || false;
  });
